...
class TestAnswersExercise14Numeric:
    """Exercise 14 numeric table — tolerance ±0.05 against the actually-trained
    models. Skipped when the runtime training environment is unavailable."""

    @staticmethod
    def _expected_metrics():
        """Return expected (accuracy, macro_f1, worst_recall) for each model.

        Trains the three models with seed 42 and computes metrics on the val
        loader. Returns a dict keyed by model name.
        """
        ...
        from classification import (
            set_seed, build_dataloaders, SimpleCNN, build_resnet18,
            run_epoch, compute_class_weights, WBC_CLASS_NAMES,
        )
        ...
        train_loader, val_loader, _, class_names = build_dataloaders(
            image_dir=images_dir, labels_csv=labels_csv,
            val_fraction=0.2, test_fraction=0.2, batch_size=32, seed=42,
        )
        ...
        df["label"] = df["label"] - 1
        counts = {WBC_CLASS_NAMES[lbl]: int(c) for lbl, c in df["label"].value_counts().sort_index().items()}
        weights = compute_class_weights(counts)
        wt = torch.tensor([weights[n] for n in WBC_CLASS_NAMES], dtype=torch.float32, device=device)

        def _train_and_eval(model, criterion, lr, epochs=30):
            model = model.to(device)
            opt = torch.optim.Adam(model.parameters(), lr=lr)
            for _ in range(epochs):
                run_epoch(model, train_loader, criterion, opt, device)
            _, acc, t, p = run_epoch(model, val_loader, criterion, None, device)
            rep = classification_report(t, p, target_names=class_names,
                                        output_dict=True, zero_division=0)
            worst = min(rep[c]["recall"] for c in class_names)
            return acc, rep["macro avg"]["f1-score"], worst

        unweighted_crit = nn.CrossEntropyLoss()
        weighted_crit   = nn.CrossEntropyLoss(weight=wt)

        return {
            "simple_unw": _train_and_eval(SimpleCNN(num_classes=4), unweighted_crit, 1e-3),
            "simple_w":   _train_and_eval(SimpleCNN(num_classes=4), weighted_crit, 1e-3),
            "resnet":     _train_and_eval(build_resnet18(num_classes=4), unweighted_crit, 1e-5),
        }
    ...
    def expected(self):
        try:
            ...
            pytest.skip(f"could not train reference models: {e}")

    # Tolerances. Accuracy and macro-F1 are averaged over many predictions and
    # are stable across runs to ~0.02; ±0.05 is a safe margin. Worst-class recall
    # is the painful one: after the 60/20/20 split a minority class can have
    # only ~10 cells in the val set, so recall is *quantized* in ~0.10 steps --
    # a single different prediction (which can come from CPU-vs-GPU floating-
    # point order, slight CUDA non-determinism, or a different augmentation
    # draw) flips the score by 0.10. Tightening this would punish students
    # whose hardware differs from the reference machine even when their model
    # is correct, so we use a wider band specifically for recall.
    TOL     = 0.05
    TOL_REC = 0.10

    def test_simple_unweighted(self, submission, expected):
        e = expected["simple_unw"]
        assert abs(_require(submission, "Q14_SIMPLE_UNW_ACC") - e[0]) <= self.TOL
        assert abs(_require(submission, "Q14_SIMPLE_UNW_F1")  - e[1]) <= self.TOL
        assert abs(_require(submission, "Q14_SIMPLE_UNW_REC") - e[2]) <= self.TOL_REC

    def test_simple_weighted(self, submission, expected):
        e = expected["simple_w"]
        assert abs(_require(submission, "Q14_SIMPLE_W_ACC") - e[0]) <= self.TOL
        assert abs(_require(submission, "Q14_SIMPLE_W_F1")  - e[1]) <= self.TOL
        assert abs(_require(submission, "Q14_SIMPLE_W_REC") - e[2]) <= self.TOL_REC

    def test_resnet(self, submission, expected):
        e = expected["resnet"]
        assert abs(_require(submission, "Q14_RESNET_ACC") - e[0]) <= self.TOL
        assert abs(_require(submission, "Q14_RESNET_F1")  - e[1]) <= self.TOL
        assert abs(_require(submission, "Q14_RESNET_REC") - e[2]) <= self.TOL_REC
